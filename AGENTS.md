...
scripts/                      — 7-stage asset extraction pipeline
  ...
  3-vision-inspect.ts         — Codex vision auto-metadata
...
JSONL transcripts at `~/.claude/projects/<project-hash>/<session-id>.jsonl`, `~/.codex/projects/<project-hash>/<session-id>.jsonl`, and `~/.Codex/projects/<project-hash>/<session-id>.jsonl` fallback on Windows. Project hash = workspace path with `:`/`\`/`/` → `-`.
